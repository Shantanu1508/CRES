...
import { DealFunding } from "../core/domain/dealfunding";
import { Amort, AutoSpreadRule, DealAdjustedTotalCommitmentTab, DealAmortization, deals, NoteAmortization, Notefunding, AutoEquity, PrepaymentPremium, SpreadMaintenanceSchedule } from "../core/domain/deals";
import { Document } from "../core/domain/document";
import { Module } from "../core/domain/Module";
import { DownloadCashFlow, Note } from "../core/domain/note";
import { NoteCashflowsExportDataList } from "../core/domain/noteCashflowsExportDataList";
import { notedetailfunding } from "../core/domain/notedetailfunding";
...
import { feeconfigurationService } from '../core/services/feeconfigurationService';
...
import { DrawFeeInvoiceDetail } from '../core/domain/DrawFeeInvoiceDetail';
import { WFService } from '../core/services/WFService';
...
export class DealDetailComponent extends Paginated implements OnInit {
    ...
    isReadonly = true;
    _isnotelevelChecked = true;
    _isincludefeeChecked = true;
    _showHidenote = false;
    private _deal: deals;
    private _dealArchieve: deals;
    private _dealext: deals;
    ...
    public CalcPrepayData: PrepaymentPremium;
    ...
    isMaturityDataChanged: boolean = false;
    ...
    private Notedetailfunding: notedetailfunding;
    private note: Note;
    private property: Property;
    private dealfunding: DealFunding;
    ...
    private PropupdatedRowNo: any = [];
    private ProprowsToUpdate: any = [];
    private SequenceupdatedRowNo: any = [];
    private SequencerowsToUpdate: any = [];
    public ScenarioId: any;
    private Dealfuturefunding: deals;
    private ShowHideFlagNoteSequence: boolean = false;
    public _isShowLoader: boolean = false;
    private _isShowHome: boolean = false;
    private _isShowNotePayrule: boolean = false;
    private _isShowDocImport: boolean = false;
    private _isShowDealRules: boolean = false;
    private _isShowDealAmort: boolean = false;
    private _isShowPayoff: boolean = false;
    private _isShowFeeInvoice: boolean = false;
    ...
    private showEquitysummary: boolean = false;
    ...
    private _ShowSucessdiv: boolean = false;
    private _ShowSucessdivMsg: string = '';
    private _isShowScenariodiv: boolean = false;
    private _isShowbtnResetdiv: boolean = false;
    private _isShowRuleTypediv: boolean = false;
    private _dealExistMsg: string = '';
    private _lstNoteExistMsg: any;
    private lrowsToUpdate: any = [];
    private _isListFetching: boolean;
    private _disabled: boolean = true;
    private _totalDealFunding: any;

    private _totalNoteFunding: any;
    private _note: Note;
    private _noteRule: Note;
    private _noteCashflowsExportDataList: NoteCashflowsExportDataList;
    private _isdealCopied: boolean;
    private _isShowCopyDeal: boolean = false;
    private _isShowDealCalcPayoffPenalty: boolean = false;
    ...
    prevdealfund: any;
    private _isShowSaveDeal: boolean = false;
    private _isShowCalcScript: boolean = false;
    ...
    private _isFundingruleChanged: boolean = false;
    ...
    private maxMaturity: any;
    private maxFullyExtendedMaturity: any;
    private minClosingDate: any;
    //Doc Import
    ...
    private _isDocumentListFetching: boolean = false;
    lstDocuments: any;
    ...
    public _SpreadMaintenance: SpreadMaintenanceSchedule;
    ...
    resultNote: any;
    private _allowRules: boolean = false;
    ...
    private _disabledTotalCommitment: boolean = true;
    private _disabledTotalCommitmentAdjustment: boolean = true;
    private _disabledComment: boolean = true;
    private _isShowAddFundingSequence: boolean = false;
    private _isShowAddRePaymentSequence: boolean = false;
    private _isReadOnlyFundingRules: boolean = true;
    noteFundingId: any;
    _isvalidateHolidaySatSun: boolean = true;
    lstLiborSchedule: any;
    ShowUseRuleN: boolean = false;
    private dealfundingColPositionDate = "2";
    private dealfundingColPositionAmount = "3";
    private dealfundingColPositionPurpose = "6";
    private dealfundingColPositionWire = "7";
    private dealfundingColPositionComment = "10";
    private _isShowDownloadExportToServicer: boolean = false;
    public _user: User;
    ...
    private _autospreadrule: AutoSpreadRule;
    ...
    private _isShowSaveDealAllowForThisRole: boolean = false;
    ...
    private showBSMsg: string = '';
    private _ShowBSData: boolean = false;
    ...
    public _lstUpdatedEquitySummary: any = [];
    ...
    private ShowHideFlagMaturitySchedule: boolean = false;
                    ...
                    this.flex.itemFormatter = function (panel, r, c, cell) {
                        ...
                        if (c == colindex) {
                            if (dealid == "00000000-0000-0000-0000-000000000000") {
                                ...
                                let item = panel.getCellData(r, c);
            ...
            this.dealSrv.GenerateFutureFunding(_objDeal).subscribe(res => {
                if (res.Succeeded) {
                    ...
                    if (header.includes("DealFundingID")) {
                        ...
                        var splength = this.dynamicColList.length - 33;
                        this.dynamicColList.splice(33, this.dynamicColList.length - 33);
        ...
        if (sDate.getFullYear() < 2000) {
            ...
            if (sDate) {
                ...
                while (i < 7) {
                    var daycnt = sDate.getDay();
                    var formateddate = this.convertDateToBindable(sDate);
                    if (daycnt == 6 || daycnt == 0 || this.ListHoliday.filter(x => this.convertDateToBindable(x.HolidayDate) == formateddate && x.HolidayTypeID == 411).length > 0) {
                        sDate.setDate(sDate.getDate() - 1);
                        isCompleted = true;
                    }
                    else {
    ...
    celleditautospread(flexautospread: wjcGrid.FlexGrid, e: wjcGrid.CellEditEndingEventArgs) {
        ...
        if (sdate) {
            ...
            if (startday == 6 || startday == 0
                || this.ListHoliday.filter(x => this.convertDateToBindable(x.HolidayDate) == formatedSdate && x.HolidayTypeID == 411).length > 0
            ) {
        ...
        if (Edate) {
            ...
            if (Endday == 6 || Endday == 0
                || this.ListHoliday.filter(x => this.convertDateToBindable(x.HolidayDate) == formatedEdate && x.HolidayTypeID == 411).length > 0
            ) {
    ...
    TotalCommitmentGridbind(listdata: any, mode: string) {
        ...
        this.flexadjustedtotalcommitment.itemFormatter = function (panel, r, c, cell) {
            ...
            if (user.RoleName == "Asset Manager") {
                ...
                let item = panel.getCellData(r, columnName);
    ...
    PrePayRuleTypeChange(newvalue): void {
        if (newvalue == 0) {
        ...
        this.dealSrv.getAllReserveSchedule(this._deal.DealID).subscribe(res => {
            if (res.Succeeded) {
                ...
                setTimeout(() => {
                    ...
                    if (colMaturityType) {
                        colMaturityType.showDropDown = true;
                        colMaturityType.dataMap = this._buildDataMap(this.lstReserveSchedulePurposeType);
                    }
        ...
        if (this.flexReserveSchedule) {
            this.flexReserveSchedule.itemFormatter = function (panel, r, c, cell) {
                ...
                if (panel.rows[r].dataItem) {
                    ...
                    let item = panel.getCellData(r, wireconfirmcol);
                    if (item == true) {
                        cell.style.backgroundColor = '#cfcfcf';
                    ...
                    if (c >= dynamiccolindex) {
                        let cellvalue = panel.getCellData(r, c);
                        if (cellvalue > 0) {
...
@NgModule({
    imports: [FormsModule, CommonModule, RouterModule.forChild(routes), WjCoreModule, WjGridModule, WjInputModule, WjGridFilterModule, WjGridDetailModule, Ng2FileInputModule.forRoot()], //, WjGridGrouppanelModule
