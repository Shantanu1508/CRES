...
export class NoteAdditionalList {
  NoteId!: string;
  ModuleId!: number;
  noteobj!: Note;
...
export class NotePrepayAndAdditionalFeeSchedule {
  ...
  ApplyTrueUpFeatureID!: number;
  ApplyTrueUpFeatureText!: string;
  PercentageOfFeeToBeStripped!: number;
}
...
export class NotePeriodicOutputsDataContract {
  ...
  UpdatedDate!: Date;
  CleanCost!: number;
  GrossDeferredFees!: number;
  DeferredFeesReceivable!: number;
  CleanCostPrice!: number;
  AmortizedCostPrice!: number;
  AdditionalFeeAccrual!: number;
  CapitalizedCostAccrual!: number;
