...
export class SelectUserComponent implements OnInit {
  ...
  @Input() set _userNameList(obj:any) {
    if (obj != undefined && obj != null) {
      this.userNameList = obj;
    }
  }

  @Input() set _returnToImpersonator(obj:any) {
    if (obj != undefined && obj != null) {
      this.userNameList = obj;
  ...
  onSelectionChange(DelegatedUserID:any, userid:any): void {
  ...
  DelegateUser() {
    ...
    this.userService.ImpersonateUserByUserID(this.userdelegateconfiguration).subscribe(res => {
      ...
      /* Saving information of Impersonator */
        ...
        localStorage.setItem('impersonatorUserInfo', user);
  ...
  ClosePopUp(): void {
    var modal:any = document.getElementById('myModelimpersonate');
  ...
  ReturnToImpersonator() {
    ...
    var user: any = localStorage.getItem('user');
    var delegateuser = JSON.parse(impersonateuser); //UserID : c41205eb-2bf6-48a3-abc3-398face6fd6e
    ...
    this.userService.ImpersonateUserByUserID(this.userdelegateconfiguration).subscribe(res => {
      if (res.Succeeded) {
        ...
        this._user.Token = res.Token;
        this._user.TokenUId = res.UserData.UserID;
        var impersonateuserinfo: any = localStorage.getItem('impersonatorUserInfo');
        this._user = JSON.parse(impersonateuserinfo);
        localStorage.setItem('user', JSON.stringify(this._user));
        localStorage.setItem('rolename', this._user.RoleName);
        localStorage.setItem('showWarningMsgdashboard', "null");
        localStorage.setItem('WarmsgdashBoad', "null");
