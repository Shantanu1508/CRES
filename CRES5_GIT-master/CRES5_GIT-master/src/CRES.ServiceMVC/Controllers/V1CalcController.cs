...
#pragma warning restore CS8632 // The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.
        ...
        [Route("api/v1calc/testfileoutput")]
...
#pragma warning restore CS0219 // The variable 'SavingFailedFor' is assigned but its value is never used
            ...
            catch (Exception ex)
            {
                ...
                string result = "";
                V1CalcLogic v1logic = new V1CalcLogic();
                result = "Saving Failed for : Transactions_Output : " + ex.Message;
                if (result.Contains("A task was canceled"))
                {
                    status = "Retry";
...
#pragma warning restore CS0219 // The variable 'SavingFailedFor' is assigned but its value is never used
        ...
        [HttpGet]
        [Services.Controllers.DeflateCompression]
        [Route("api/v1calc/checkCalculationStatus")]
        public void CheckCalculationStatus()
        {
            ...
            try
            {
                ...
                string headerkey = Sectionroot.GetSection("Authkeyname").Value;
                string headerValue = Sectionroot.GetSection("Authkeyvalue").Value;
                string ApiConstantUrl = Sectionroot.GetSection("V1ApiConstantUrl").Value;
                V1CalcLogic v1logic = new V1CalcLogic();
                // get all deal with processing  in database
                List<V1CalculationStatusDataContract> listdeals = v1logic.GetAllDealsProcessingstatus();
                ...
                if (listdeals != null)
                {
                    ...
                    {
#pragma warning disable CS0219 // The variable 'res1' is assigned but its value is never used
                        string res1 = "";
#pragma warning restore CS0219 // The variable 'res1' is assigned but its value is never used
                        requestid = item.RequestID;
                        apiPath = "requests/" + requestid + "/status";
                        System.Net.ServicePointManager.SecurityProtocol |= SecurityProtocolType.Tls12;
                        using (var client = new HttpClient())
                        {
                            client.DefaultRequestHeaders.Add(headerkey, headerValue);
                            var res = client.GetAsync(ApiConstantUrl + apiPath);
                            try
                            {
                                ...
                                if (response1.IsSuccessStatusCode)
                                {
                                    ...
                                    var _jsonres = JObject.Parse(response);
                                    if (_jsonres["status"].ToString() == "2")
                                    {
                                        Thread FirstThread = new Thread(() => GetFileOutput(item.RequestID));
                                        FirstThread.Start();
                                    }
                                    else if (_jsonres["status"].ToString() == "-1" || _jsonres["status"].ToString() == "-2")
                                    {
                                        var result = _jsonres["additional_information"].ToString();
                                        edc = ReadException(result);
                                        Log.WriteLogExceptionMessage(CRESEnums.Module.Calculator.ToString(), "Error in calculating for request id:: " + requestid + " " + edc.StackTrace, requestid, "B0E6697B-3534-4C09-BE0A-04473401AB93", edc.MethodName, edc.Summary);
                                    }
                                    v1logic.UpdateCalculationRequestsStatus(item.DealID, item.RequestID, Convert.ToInt32(_jsonres["status"]), item.AnalysisID, "00000000-0000-0000-0000-000000000000", edc.StackTrace);
                                }
                            }

                            catch (Exception ex)
                            {
                                Log.WriteLogException(CRESEnums.Module.Calculator.ToString(), "Error in checking status from api ", "", "B0E6697B-3534-4C09-BE0A-04473401AB93", "CheckCalculationStatus", "", ex);
                            }
                        }
                    }
                }
            ...
            {
                Log.WriteLogException(CRESEnums.Module.Calculator.ToString(), "Error in CheckCalculationStatus ", "", "B0E6697B-3534-4C09-BE0A-04473401AB93", "CheckCalculationStatus", "", ex);
            }
        }
...
#pragma warning disable CS0168 // The variable 'ex' is declared but never used
            try
            {
                ...
                if (apiresult.IsSuccessStatusCode == true)
                {
                    ...
                    var isJSONSaved = Services.AzureStorageReadFile.UploadJSONFileToAzureBlob(dfresponse, fileName);
...
#pragma warning disable CS0168 // The variable 'ex' is declared but never used
            ...
            {
                //    V1CalcLogic v1logic = new V1CalcLogic();
                //string status=    v1logic.UpdateM61EngineCalcStatus(requestid, Convert.ToInt32(2), "");
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
        ...
        [HttpGet]
        [Services.Controllers.DeflateCompression]
        [Route("api/v1calc/geterrors")]
        public void geterrors(string requestid)
        {
#pragma warning disable CS0219 // The variable 'res1' is assigned but its value is never used
            string res1 = "";
#pragma warning restore CS0219 // The variable 'res1' is assigned but its value is never used
#pragma warning disable CS0168 // The variable 'ex' is declared but never used
            try
            {
                ...
                {
                    {
                        // requestid = "2e016e58ffc143cfa26c2fe614f6ce89";
                        apiPath = "requests/" + requestid + "/status";
                        System.Net.ServicePointManager.SecurityProtocol |= SecurityProtocolType.Tls12;
                        using (var client = new HttpClient())
                        {
                            ...
                            try
                            {
                                ...
                                if (response1.IsSuccessStatusCode)
                                {
                                    ...
                                    if (_jsonres["status"].ToString() == "2")
                                    {
                                        Thread secondThread = new Thread(() => GetFileOutput(requestid));
                                        secondThread.Start();
                                    }
                                    else if (_jsonres["status"].ToString() == "-1" || _jsonres["status"].ToString() == "-2")
                                    {
                                    ...
                                    v1logic.UpdateCalculationRequestsStatus("51f8cb3d-1428-4c19-9ea7-6c7fe2029544", requestid, Convert.ToInt32(_jsonres["status"]), analysisID, "00000000-0000-0000-0000-000000000000", edc.StackTrace);
