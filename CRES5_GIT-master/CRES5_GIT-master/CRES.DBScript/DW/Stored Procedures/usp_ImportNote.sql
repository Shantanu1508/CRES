...
CREATE PROCEDURE [DW].[usp_ImportNote]
...
BEGIN
	...
	VALUES (@BatchLogId,'L_NoteBI',GETDATE())

	DECLARE @id int,@RowCount int
	SET @id = (SELECT @@IDENTITY)


IF EXISTS(Select Noteid from [dw].[NoteBI])
BEGIN
...
n.[ServicerID],
n.[TotalCommitment],
LDeterminationDateHolidayListBI.CalendarName as  [DeterminationDateHolidayListBI],
...
lInterestCalculationRuleForPaydownsAmort.name as InterestCalculationRuleForPaydownsAmortBI,
...
Left JOin(
	...
	WHERE ac.isdeleted <> 1 and (n.CreatedDate > @LastBatchStart 
				and n.CreatedDate < @CurrentBatchStart) 
				OR 
				(n.UpdatedDate > @LastBatchStart 
				and n.UpdatedDate < @CurrentBatchStart)


SET @RowCount = @@ROWCOUNT
Print(char(9) +'usp_ImportNote - ROWCOUNT = '+cast(@RowCount  as varchar(100)));
...
SET @RowCount = @@ROWCOUNT
Print(char(9) +char(9)+'usp_ImportNote - ROWCOUNT = '+cast(@RowCount  as varchar(100)));
