...
--[dbo].[usp_GetNoteDealAmortizationScheduleDealIDFromStartEndDate]  'b0e6697b-3534-4c09-be0a-04473401ab93',  '0F813461-56CF-48B8-B666-3D1F779A2D94',619,'3/7/2018','9/7/2020'

CREATE PROCEDURE [dbo].[usp_GetNoteDealAmortizationScheduleDealIDFromStartEndDate]  
(
...
DECLARE @DBAmortMethod int =(select AmortizationMethod from cre.deal where dealid = @DealID )
	...
	SET @ColPivot = STUFF((SELECT  ',' + QUOTENAME(cast(acc.Name as nvarchar(256)))                   
		...
		,a.DealID	
		,getdate() as DateWithoutAdjustment
		,a.Date	
		,a.Amount 
		...
		)b on a.dealid = b.dealid and a.date = b.date where a.Date between ''' + CAST(@StartDate  AS VARCHAR(100))   + ''' and ''' + CAST(@EndDate  AS VARCHAR(100))   + '''			
		'
