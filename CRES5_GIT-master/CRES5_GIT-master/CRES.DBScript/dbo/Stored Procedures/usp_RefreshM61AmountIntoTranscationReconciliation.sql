...
From(
	Select Transcationid,New_M61_Amount,Cal_Delta,Cal_ActualDelta  
	 From(  
		  ...
		  left join(  
			Select noteid,Date,Amount,[Type] from cre.transactionEntry where analysisID =  @AnalysisID
			and NoteID in (Select noteid from [#UnReconRecord] )
		  ...
		  and nt.TransactionType='InterestPaid' 
	 )a
...
IF OBJECT_ID('tempdb..#TempTransactionEntry') IS NOT NULL                                             
 ...
 and NOteid in (select noteid from cre.TranscationReconciliation where posteddate is null and Deleted <> 1  )  
