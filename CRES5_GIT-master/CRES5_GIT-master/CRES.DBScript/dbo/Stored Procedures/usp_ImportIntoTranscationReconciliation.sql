...
--[dbo].[usp_ImportIntoTranscationReconciliation] 1035,'c10f3372-0fc2-4861-a9f5-148f1f80804f'    
...
INSERT INTO #TempTransaction(LandingID,NoteID,TransactionType,Transcationtypeid,ServicingAmount,DueDate,RemitDate,ServcerMasterID,TotalRemit,TranDate,AddlInterest,Exception,InterestAdj,ShouldDelete)      
SELECT LandingID,n.noteid,TrType, (Select LookupID from core.lookup where parentid=39 and [Name] = TrType) as TrTypeLokkupId,Ser_Amount as ServicingAmount,DueDate,RemitDate,ServcerMasterID,TotalRemit,TranDate,AddlInt,Exception,InterestAdj ,0 as ShouldDelete    
FROM     
(    
 Select LandingID, NoteID,DueDate,NextPmtDue,Principal as [ScheduledPrincipalPaid],Interest as [InterestPaid],RemitDate,(SELECT ServcerMasterID FROM [IO].[FileBatchLog] WHERE BatchLogID=@Batchlogid) as ServcerMasterID,ExitFees,TotalRemit,TranDate,AddlInt,
...
and TransactionType = 'InterestPaid'   
 ...
 from cre.TransactionAuditLog ta 
    where  ta.TransactionType = 'InterestPaid'  and    DueDate is null 
 and ta.Batchlogid =@Batchlogid 
...
--where ttr.Noteid not in (Select Noteid from cre.TransactionAuditLog where [Status] in ('Note does not exist.','Data already Reconcilled.','Zero Interest') and Batchlogid = @Batchlogid)    
--and deleted=0    
--and  tr.deleted=0    
--and tr.Transcationid is null or tr.deleted=0)    
