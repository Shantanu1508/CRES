...
CREATE PROCEDURE [dbo].[usp_LCGetServicingFeeScheduleByCRENoteID] --'N09262016'
(
...
PRINT(@cols);
  ...
  FROM (
	...
	FROM (			
			Select 
			cast((ROW_NUMBER() over (order by sfs.ServicingFeeScheduleID))  as varchar(50))   as [RowCount]
			...
			,ISNULL(Cast(sfs.Value as nvarchar(MAX)),'''') as [Value]
