...
(        
 ...
 Select [Transcationid] from @TmpreconTrans --where [Transcationid]  ='C637A832-1A23-44D1-966C-6B1509DF681A'        
)        
OPEN CursorFeeRecon         
...
set @SevenDaysPrevious=(Select Date as enddate from @tblDateRange  where date < @RemittanceDate order by date OFFSET 6 ROWS FETCH NEXT 1 ROWS ONLY)      
...
@DealId as DealId,        
@NoteID as NoteID,       
(case when @TransactionType like '%exit%' Then 'ExitFeeExcludedFromLevelYield' when @TransactionType like '%extension%' Then 'ExtensionFeeExcludedFromLevelYield' when @TransactionType ='PrepaymentFee' Then 'PrepaymentFeeExcludedFromLevelYield'          
...
UpdatedBy,UpdatedDate,TransactionTypeText,TranscationReconciliationID,RemittanceDate,Exception,Adjustment,ActualDelta,OverrideReason,InterestAdj,AddlInterest,TotalInterest,[BerAddlint])            
Select [NoteID]        
,[TransactionDate]        
