   ...
   select tbl1.DealName,tbl1.CREDealID,[InvoiceDetailID]
	...
	(
	  ...
	  FROM [CRE].[InvoiceDetail] i
		...
		and (case when len(d.DealName)>41 then d.CREDealID+'-'+substring(d.DealName,1,41 - len(d.CREDealID+'-')) else d.DealName end) not in 
		(
		select FullName from app.quickbookcustomer
		)
		) tbl1
		join
		(
