CREATE PROCEDURE [dbo].[usp_GetTranscationForReconciliation] 
AS
BEGIN
	...
	select	
		...
		d.DealID,
			...
			When ISNULL(tr.OverrideValue,0)<>0  Then tr.OverrideValue-tr.CalculatedAmount
			END as Delta,
			ISNULL(tr.Adjustment,0) as Adjustment,
			ISNULL(tr.ActualDelta,tr.Delta) as ActualDelta,
			ISNULL(tr.AddlInterest,0) as AddlInterest,
			ISNULL(tr.TotalInterest,0) as TotalInterest,
