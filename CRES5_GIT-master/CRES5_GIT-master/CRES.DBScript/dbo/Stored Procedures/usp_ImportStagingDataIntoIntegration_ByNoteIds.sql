...
CREATE TABLE #tblListNotes(CRENoteID VARCHAR(256),NoteID UNIQUEIDENTIFIER)

INSERT INTO #tblListNotes(CRENoteID,NoteID)
Select Value ,n.noteid
from fn_Split(@MultipleNoteids) fn
inner join cre.note n on n.crenoteid = fn.Value


Declare @NoteCommaSperated nvarchar(max);  
   
 --Create comm separted notes   
 Select @NoteCommaSperated = STUFF((SELECT  ',''' + cast(crenoteid as nvarchar(256))  +''''                       
         from #tblListNotes  
   FOR XML PATH(''), TYPE        
      ).value('.', 'NVARCHAR(MAX)')         
     ,1,1,'')  
...
exec [dbo].[usp_Import_Staging_Note_ByMultipleNoteID] @MultipleNoteids
...
where noteid in (Select noteid from #tblListNotes)
...
--and n.noteid in (Select noteid from #tblListNotes)
...
--Where tr.AnalysisID = @analysiID
--and tr.noteid in (Select noteid from #tblListNotes)
...
From(
	...
	where cf.noteid in (Select noteid from #tblListNotes)
)a
...
Delete from DW.Staging_IntegartionCashFlowBI where NoteKey in (Select noteid from #tblListNotes)
...
where In_np.NoteKey in (Select noteid from #tblListNotes)
...
From(
    ...
    Where n.noteid in (Select noteid from #tblListNotes)
)a
