CREATE PROCEDURE [dbo].[usp_GetDiscrepancyForFFBetweenM61andBackshop] 
AS
BEGIN
	SET NOCOUNT ON;
	...
	from [acore].[vw_AcctNoteFundings] where FundingDate > ''12/31/2019'' and FundingPurposeCD_F not in (''PIKNC'',''PIKPP'') and NoteID_f in (Select Distinct noteid from [acore].[vw_AcctNote])  '  
	...
	-----======Delete paydowns having no comment======
	--Delete FROM @M61FF where NULLIF(Comments,'') is NULL and FundingPurpose = 'PAYDOWN'
	----=========================================
	...
	Select * from(
		Select  dealname as [Deal Name],
				...
				CRENoteID,
				CONVERT(varchar, M61_FundingDate, 101) as M61_FundingDate,
				M61_FundingAmount,
				CONVERT(varchar, BS_FundingDate, 101) as BS_FundingDate,
