--drop  PROCEDURE [dbo].[usp_UpdateTranscation]      
...
INSERT into cre.NoteTransactionDetail(NoteID,TransactionDate,TransactionType,Amount,RelatedtoModeledPMTDate,ServicingAmount,CalculatedAmount,Delta,M61Value,ServicerValue,Ignore,OverrideValue,comments,PostedDate,ServicerMasterID,CreatedBy,CreatedDate,UpdatedBy,UpdatedDate,TransactionTypeText,TranscationReconciliationID,RemittanceDate,Exception,Adjustment,ActualDelta,OverrideReason,InterestAdj,AddlInterest,TotalInterest)      
Select       
 tre.NoteID,       
 tre.TransactionDate,      
 (      
 ...
 and TransactionType not in ('PIKInterestPaid','ManagementFee','ExitFee','ExtensionFee','PrepaymentFee','UnusedFee')      
...
where TransactionType not in ('InterestPaid','ScheduledPrincipalPaid')  
  and (isnull(M61Value,0) <>0 or isnull(ServicerValue,0) <>0  or OverrideValue is not null)      
