...
SELECT NoteID,Date,Amount,Type,AnalysisID,FeeName
...
select Distinct np.noteid,PeriodEndDate,
(CASE WHEN (nn.InitialFundingAMount = 0.01 and ISNULL(np.EndingBalance,0) <> 0) THEN ISNULL(np.EndingBalance,0) - ISNULL(nn.InitialFundingAMount,0)
