...
,@CalcEngineType int  
,@AllowGaapComponentInCashflowDownload int  
,@WatchlistStatus nvarchar(256)  
,@EnableAutoDistributePrincipalWriteoff bit
,@PrepaymentGroupSize int  
,@PrepaymentAllocationMethod int 
,@MaturityAdjMonthsOverride int = NULL
,@ExcludeDealFromLiability bit = null
,@LiabilitySource int,
@InternalRefi int ,
@PortfolioLoan int,
...
VALUES(1,@DealName,10,@CreatedBy,GETDATE(),@UpdatedBy,GetDATE(),0)        
...
INSERT INTO CRE.Deal          
 ...
 ,CalcEngineType  
 --,LoanStatusID    
 ,AllowGaapComponentInCashflowDownload    
 ,WatchlistStatus  
 ,EnableAutoDistributePrincipalWriteoff
 ,AccountID  
 ,PrepaymentGroupSize
 ,PrepaymentAllocationMethod
 ,MaturityAdjMonthsOverride
 ,ExcludeDealFromLiability
 ,LiabilitySource
 ,InternalRefi
,PortfolioLoan
...
,NettingofReservesEscrows
  ...
  OUTPUT inserted.DealID INTO @tDeal(tNewDealId)          
   ...
   ,@LatestPossibleRepaymentDate           
   ,@Blockoutperiod           
   ,NULLIF(@PossibleRepaymentdayofthemonth,0)  
   ,NULLIF(@Repaymentallocationfrequency,0)       
   ,@EnableAutoSpreadRepayments        
   --,@KnownFullPayOffDate        
   ,@DealLevelMaturity        
   ,@ApplyNoteLevelPaydowns        
   ,@IsREODeal        
   ,ISNULL(@BalanceAware,0)  
   ,@PrepayDate   
   ,@ICMFullyFundedEquity  
   ,@EquityatClosing  
   ,@CalcEngineType  
 ...
 EXEC [dbo].[usp_UpdateTransactionEntryBalloonRepayAmountForEnableM61CalcN] @NewDealID
...
--Log activity end          
  ...
  SEt @L_Accountid = (Select AccountID from CRE.Deal where DealID = @DealID)  
...
Update CRE.Deal set           
    ...
    PossibleRepaymentdayofthemonth= NULLIF(@PossibleRepaymentdayofthemonth,0),          
    Repaymentallocationfrequency= NULLIF(@Repaymentallocationfrequency,0),          
 ...
 --KnownFullPayOffDate = @KnownFullPayOffDate        
    ...
    ApplyNoteLevelPaydowns = @ApplyNoteLevelPaydowns,        
	 IsREODeal = @IsREODeal,        
	 BalanceAware=ISNULL(@BalanceAware,0) ,    
	 ...
	 AllowGaapComponentInCashflowDownload = @AllowGaapComponentInCashflowDownload,  
	 WatchlistStatus = @WatchlistStatus,
	 EnableAutoDistributePrincipalWriteoff = @EnableAutoDistributePrincipalWriteoff
	 ,PrepaymentGroupSize=@PrepaymentGroupSize   
	,PrepaymentAllocationMethod=@PrepaymentAllocationMethod    
	,MaturityAdjMonthsOverride = @MaturityAdjMonthsOverride
	,ExcludeDealFromLiability=@ExcludeDealFromLiability
 ...
 EXEC [dbo].[usp_UpdateRealizedUnRealizedDealLevel]  @NewDealID
