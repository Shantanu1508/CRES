...
-- View    
-- View    
CREATE VIEW [dbo].[vw_NPC_AllScenarios_Unpivot]      
AS      
    
    
Select noteid as NoteKey,crenoteid as NoteID,PeriodEndDate as [Date],[Value] as Amount,[Type],AnalysisID,Scenario,dealname,credealid as DealID,NoteName    
From(      
 Select n.noteid,a.Name as Scenario,d.dealname,d.credealid,n.crenoteid,  nc.AnalysisID,acc.name as NoteName,nc.PeriodEndDate       
 --,ISNULL(ActualCashFlows,0) as ActualCashFlows      
 --,ISNULL(GAAPCashFlows,0) as GAAPCashFlows      
 ,ISNULL(EndingGAAPBookValue,0) as EndingGAAPBookValue_NPC    
