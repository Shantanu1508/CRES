CREATE VIEW [dbo].[vw_Recon_Final_PeriodicOutput_Int_Vs_Stg]  
 AS  
...
SUM(a.Value) as DevAmount ,SUM(b.Value) as StgAmount, (ISNULL(SUM(a.Value),0)- ISNULL(SUM(b.Value),0)) as Delta,  n.closingDate as ClosingDate, 
n.ActualPayoffDate, DevCalcEngineType, ISNULL(c.Status,0) as CalculationStatus, ISNULL(a.Scenario,NULL) as [DevScenario], 
ISNULL(b.Scenario,NULL) as [StgScenario]
from (
	Select * from [dbo].[vw_NotePeriodicCalc_Unpivot_Default]
	
) a
full outer join (
	Select * from dbo.vw_NotePeriodicCalc_Unpivot_Staging
