...
WHERE [TYPE] in( 'PIKPrincipalPaid') and Scenario = 'Default'  and AccountTypeID = 1
GROUP BY [NoteId], CONVERT(nvarchar(6), [Date], 112)  
),  
PIKPrincipalFunding as  
(   
SELECT NoteID, CONVERT(nvarchar(6), [Date], 112) [MonthYear], sum(Amount) Amount from TransactionEntry   
WHERE [TYPE] in( 'PIKPrincipalFunding') and Scenario = 'Default'  and AccountTypeID = 1
GROUP BY [NoteId], CONVERT(nvarchar(6), [Date], 112)  
)  
...
LEFT JOIN (
		Select NoteID,[Date],AllInCouponRate,Analysisid ,[AdditionalPIKSpreadfromPIKTable],AllInPikRate
		from [CRE].[PeriodicInterestRateUsed] 
		where analysisid='C10F3372-0FC2-4861-A9F5-148F1F80804F'   
		and IsPaymentDate is NULL
) AS Rate  ON Rate.NoteID = uCashFlow.NoteKey and Rate.[Date] = uCashFlow.[PeriodEndDate]  AND Rate.AnalysisID = 'C10F3372-0FC2-4861-A9F5-148F1F80804F'
