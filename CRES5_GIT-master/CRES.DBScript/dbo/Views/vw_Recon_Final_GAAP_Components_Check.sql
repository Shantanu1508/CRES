-- View
CREATE VIEW [dbo].[vw_Recon_Final_GAAP_Components_Check]  

AS  

Select DealName, CreDealID as DealID, CreNoteid as NoteID, NoteName,
			--[Month],

			PeriodEndDate ,
			ISNULL(EndingGAAPBookValue,0) as EndingGAAPBookValue  ,

			CalculatedGAAP= (Cleancost+[AccumaltedDiscountPremiumBI]+ [CurrentPeriodPIKInterestAccrual]+[CurrentPeriodInterestAccrual]+ AccumulatedAmort+
			[AccumalatedCapitalizedCostBI]+InterestSuspenseAccountBalance ),
			
			EndingGAAPBookValue-((Cleancost+[AccumaltedDiscountPremiumBI]+ [CurrentPeriodPIKInterestAccrual]+
			[CurrentPeriodInterestAccrual]+ AccumulatedAmort+[AccumalatedCapitalizedCostBI]+InterestSuspenseAccountBalance)) as Delta,

			ABS(EndingGAAPBookValue-((Cleancost+[AccumaltedDiscountPremiumBI]+ [CurrentPeriodPIKInterestAccrual]+[CurrentPeriodInterestAccrual]+ 
			AccumulatedAmort+[AccumalatedCapitalizedCostBI]+InterestSuspenseAccountBalance))) as ABS_Delta,
			...
			ISNULL(AccumaltedDiscountPremiumBI,0) as AccumaltedDiscountPremiumBI ,
			ISNULL(AccumalatedCapitalizedCostBI,0) as AccumalatedCapitalizedCostBI
			From(
			Select
			DealName
			,CREDealID
			...
			--select  n.Account_AccountID AccountId from cre.Deal d 
			--	join cre.Note n on n.DealID=d.DealID  
			--	join core.Account a on a.AccountID=n.Account_AccountID 
			--	where   d.CalcEngineType=798 and a.IsDeleted<>1
