...
--select NoteID from cre.Note where CRENoteID='24040'
/*	Balance Check	*/
Create View vw_Recon_NoteEndingBalanceV1 as
select [Name],creDealid, DealName, CRENoteID, sum(Delta) as Delta from (
	select an.Name,creDealid, DealName, CRENoteID,npc.PeriodEndDate , npc.Month
	...
	, case 
		when Month=1 then EndingBalance  + isnull(npc.EndingPIKBalanceNotInsideLoanBalance,0) - (BeginningBalance + isnull(npc.BeginningPIKBalanceNotInsideLoanBalance,0) + InitialFundingAmount + TotalFutureAdvancesForThePeriod + TotalDiscretionaryCurtailmentsforthePeriod + ScheduledPrincipal  + isnull(PrincipalPaid,0) + BalloonPayment + (PIKInterestAppliedForThePeriod + isnull(npc.PIKInterestForPeriodNotInsideLoanBalance,0)) - (PIKPrincipalPaidForThePeriod + isnull(npc.PIKBalanceBalloonPayment,0)))
		else  EndingBalance  + isnull(npc.EndingPIKBalanceNotInsideLoanBalance,0) - (BeginningBalance + isnull(npc.BeginningPIKBalanceNotInsideLoanBalance,0) + TotalFutureAdvancesForThePeriod + TotalDiscretionaryCurtailmentsforthePeriod + ScheduledPrincipal + /* - isnull(PrincipalPaid,0)*/ + BalloonPayment + (PIKInterestAppliedForThePeriod + isnull(npc.PIKInterestForPeriodNotInsideLoanBalance,0)) + (PIKPrincipalPaidForThePeriod + isnull(npc.PIKBalanceBalloonPayment,0)) )
	 End as Delta
	...
	join (
		...
		and d.IsDeleted <> 1
		--and NoteID='C4CEEC5C-9C6F-4751-A304-F6C4E6988A8F'
	...
	--and CRENoteID in ('23524','23890','23891','23892')
	--and CRENoteID in ('12967')
	--order by an.Name, DealName, CRENoteID, npc.Month
) cf
