...
-- Procedure
CREATE PROCEDURE [dbo].[usp_UpdateXIRRDealOutputforCutOffDate] --3,'3/31/2025', 'B0E6697B-3534-4C09-BE0A-04473401AB93'
    @XIRRConfigID int,
    @CutOffDate Date,
...
BEGIN    
	...
	IF OBJECT_ID('TempDB..#temp_DealList') IS NOT NULL
		DROP TABLE #temp_DealList;

	CREATE TABLE #temp_DealList(
		DealAccountId UniqueIdentifier NULL
	)

    Insert into #temp_DealList (DealAccountId)
	SELECT DealAccountID FROM CRE.XIRROutputDealLevel WHERE XIRRConfigID = @XIRRConfigID
		...
		FROM(	
			Select * from(
				...
				INNER JOIN(					
					...
					WHERE EventTypeID = 14 AND D1.AccountID in (Select DealAccountID from #temp_DealList) AND eve.StatusID = 1 AND acc.IsDeleted = 0
				...
				WHERE e.StatusID = 1 AND D.AccountID in (Select DealAccountID from #temp_DealList) 
	...
	and d.accountid in (Select DealAccountID from #temp_DealList)
	...
	Left Join (
				...
				FROM(       
					...
					INNER JOIN CORE.account acc ON acc.AccountID = n.Account_AccountID    
					INNER JOIN #temp_DealList TDL ON TDL.DealAccountId=D.AccountID
					--INNER JOIN cre.XIRRCalculationInput XCI ON XCI.DealAccountID=D.AccountID AND N.PoolID = XCI.PoolID AND N.Account_AccountID = XCI.NoteAccountID
					INNER JOIN (
					...
					--AND XCI.XIRRConfigID=@XIRRConfigID					
	...
	Left Join (
				...
				FROM(       
					...
					AND L.[Name]='Sub' 
					AND XCI.XIRRConfigID=@XIRRConfigID
					--AND N.PoolID IN (SELECT Distinct ISNULL(PoolID,'') from cre.XIRRCalculationInput Where DealAccountID=@DealAccountID AND XIRRConfigID=@XIRRConfigID)
				)a    
				WHERE RNo =  1  
				GROUP BY DealID
	...
	Left Join (
		Select n.DealID,SUM(WeightedSpread) as 'WholeLoanSpread' 
		from CRE.Note n
		Inner join cre.deal d on d.dealid = n.dealid
		Inner Join @tblNoteIDTags tn on tn.DealID = d.DealID and tn.NoteID = n.NoteID
		Group By n.DealID
