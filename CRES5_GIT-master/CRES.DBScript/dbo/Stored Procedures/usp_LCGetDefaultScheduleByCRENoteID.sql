...
CREATE PROCEDURE [dbo].[usp_LCGetDefaultScheduleByCRENoteID] --'N09262016'
(
...
PRINT(@cols);
  ...
  FROM (
	...
	FROM (			
			Select 
			cast((ROW_NUMBER() over (order by ds.DefaultScheduleID))  as varchar(50))   as [RowCount]
			...
			,ISNULL(Cast(ds.Value as nvarchar(MAX)),'''') as [Value]
