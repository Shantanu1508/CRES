--[dbo].[usp_GetFundingDrawByBusinessdayWF] 2
CREATE PROCEDURE [dbo].[usp_GetFundingDrawByBusinessdayWF_BackDate] 
   @NextBDNumber INT
AS
BEGIN
      SET NOCOUNT ON;
...
;WITH CTE AS  
(    
	SELECT DateAdd(day,-30,getdate()) Date  
	UNION ALL
	SELECT DateAdd(day,1,Date)  FROM CTE WHERE DateAdd(day,1,Date) <= DateAdd(day,15,getdate())  
)  
...
SET @NextBDDate = (Select Date as enddate from @tblDateRange  where date <= CAST(getdate() as date) order by date desc OFFSET (@NextBDNumber) ROWS FETCH NEXT 1 ROWS ONLY)
Print(CAST(@NextBDDate as nvarchar(256)))
---=================================
...
,LTRIM(RTRIM(uAm.Firstname))+' '+LTRIM(RTRIM(uAm.lastname)) as AMUser  ---primary

,LTRIM(RTRIM(uAmSec.Email)) as AMSecondUser_Email
,LTRIM(RTRIM(uAm.Email)) as AMUser_Email  ---primary
...
--and comment is null
and df.amount > 0
and df.[date] >= @NextBDDate and df.date <= Cast(getdate() as date)
and ISNULL(tblWF.WF_CurrentStatus,'Projected') <> 'Completed'
