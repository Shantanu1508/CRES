-- Procedure
-- Procedure
CREATE PROCEDURE [dbo].[usp_GetAllInvoiceQueued_forSandbox]  
(  
 @UserID NVARCHAR(256)  
)  
AS  
...
SELECT [InvoiceDetailID]  
  ...
  IsManualInvoice=case when i.AutoSendInvoice=572 then 1 else 0 end  
   ...
   join   
   (  
  select TaskID from cre.WFTaskDetail where Comment = 'Draw Fee Invoiced'  
  --and cast(CreatedDate as date)=cast(getdate() as date)  
  and TaskTypeID=502  
  --and TaskID='164B0C0A-C0A9-4F53-869A-9CCE1083B459'  
  and TaskID not in (select TaskID from CRE.InvoiceDetailForSandbox)  
   ) inv  
   on inv.TaskID=i.ObjectID  
      where df.Applied=1   
   and ObjectTypeID=698  
   and InvoiceTypeID = 558  
   and cast([Date] as date)<=cast(getdate() as date)   
   and  i.DrawFeeStatus = 693   
   and inv.TaskID not in (select TaskID from CRE.InvoiceDetailForSandbox)  
   and  i.invoicedetailid = 7  
   END  