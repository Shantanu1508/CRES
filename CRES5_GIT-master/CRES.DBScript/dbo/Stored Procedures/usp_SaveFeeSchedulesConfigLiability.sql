-- Procedure
CREATE Procedure [dbo].[usp_SaveFeeSchedulesConfigLiability]
(
@UserID NVarchar(255),
...
BEGIN
  ...
  --insert new record
	INSERT INTO [CRE].[FeeSchedulesConfigLiability]
           ([FeeTypeNameText]
     ...
     select
		   ...
		   and FeeTypeNameText not in 
			(
			  select FeeTypeNameText from CRE.FeeSchedulesConfigLiability
			)


 --update existing record

	UPDATE [CRE].[FeeSchedulesConfigLiability]
	   ...
	   FROM
		...
		where cre.FeeSchedulesConfigLiability.FeeTypeGuID = ts.FeeTypeGuID 
