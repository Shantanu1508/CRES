...
CREATE Procedure [dbo].[usp_SaveFeeFunctionsConfigLiability]
@UserID NVarchar(255),
...
--insert new record
	INSERT INTO [CRE].[FeeFunctionsConfigLiability]
				   ([FunctionNameText]
				   ,[FunctionTypeID]
				   ,[PaymentFrequencyID]
				   ,[AccrualBasisID]
	...
	select
			...
			(
				select FunctionNameText from [CRE].[FeeFunctionsConfigLiability]
			)

 --update existing record

		UPDATE [CRE].[FeeFunctionsConfigLiability]
		   ...
		   SET [FunctionTypeID] = tf.FunctionTypeID
			...
			where cre.FeeFunctionsConfigLiability.FunctionGuID = tf.FunctionGuID 
