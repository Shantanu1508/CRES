-- Procedure
--[dbo].[usp_VerifyFutureFundingM61andBackshop_ByDealID] '4289CC39-8CDD-4E9E-9352-49B3F1BE152F',null,'B0E6697B-3534-4C09-BE0A-04473401AB93','FF'

CREATE PROCEDURE [dbo].[usp_VerifyFutureFundingM61andBackshop_ByDealID]   
(  
 @DealID nvarchar(256),  
...
BEGIN
	...
	BEGIN	
		...
		from [acore].[vw_AcctNoteFundings] where FundingDate > '''+ CAST(@Cutoffdate as nvarchar(256)) +''' and FundingPurposeCD_F not in (''PIKNC'',''PIKPP'') and NoteID_f in (Select Distinct noteid from [acore].[vw_AcctNote] where ControlId = '''+@credealid+''')  '  ---,''PAYOFF''
	...
	BEGIN	
		...
		where FundingPurposeCD_F not in (''PIKNC'',''PIKPP'')
		and PaymentDate > '''+ CAST(@Cutoffdate as nvarchar(256)) +'''
	...
	BEGIN	
		...
		---
	END
	IF(@verificationFor = 'Balloon')
	BEGIN	
		
		SET @CRENoteId = (Select crenoteid from cre.note where noteid = @noteid)
		
		SET @query2 = N'Select Distinct CAST(NoteID_f as varchar(256)),FundingDate,FundingAmount,'''' as Comments,FundingPurposeCD_F,Applied,WireConfirm,FundingDrawId   
		from [acore].[vw_AcctNoteFundings] where FundingDate > '''+ CAST(@Cutoffdate as nvarchar(256)) +''' and FundingPurposeCD_F in (''PAYOFF'')  and NoteID_f = '''+ @CRENoteId +''' ' 
	...
	END
	ELSE IF(@verificationFor = 'Balloon')
	BEGIN
		INSERT INTO @M61FF (CRENoteID,FundingDate,FundingAmount,Comments,FundingPurpose,Applied,WireConfirm,DrawFundingId)	
		Select 
		n.[CRENoteID]
		,tr.[Date] as FundingDate
		,(tr.[Amount]) as FundingAmount
		,tr.Comment as Comments
		,'Balloon' FundingPurpose		
		,1 as [Applied]
		,1 as [WireConfirm]
		,null as [DrawFundingId]
		from cre.transactionentry tr
	...
	Delete from core.Exceptions where ObjectID in (Select noteid from cre.note where dealid = @DealID) and FieldName = 'M61 and Backshop funding mismatch' and ObjectTypeID = 182 
	
	IF EXISTS(Select dealname from @tblDiscrepancy)
	BEGIN	
		IF(@verificationFor = 'PIK')
		BEGIN
			...
			Select NoteId ,182 ,'M61 and Backshop PIK Principal Funding mismatch','M61 and Backshop PIK Principal Funding mismatch',294,@userID,GETDATE(),@userID,GETDATE()	
			from @tblDiscrepancy
		END
		ELSE IF(@verificationFor = 'Balloon')
		BEGIN
			INSERT into core.Exceptions ([ObjectID],[ObjectTypeID],[FieldName],[Summary],[ActionLevelID],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate])  
			Select NoteId ,182 ,'M61 and Backshop Balloon mismatch','M61 and Backshop Balloon mismatch',294,@userID,GETDATE(),@userID,GETDATE()	
			from @tblDiscrepancy
		END
		ELSE
		BEGIN
			INSERT into core.Exceptions ([ObjectID],[ObjectTypeID],[FieldName],[Summary],[ActionLevelID],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate])  
			Select NoteId ,182 ,'M61 and Backshop funding mismatch','M61 and Backshop funding mismatch.',294,@userID,GETDATE(),@userID,GETDATE()	
			from @tblDiscrepancy	  
		END
	END
	ELSE
	BEGIN
		IF(@verificationFor = 'FF')
		BEGIN
			UPDATE [IO].[out_FutureFunding] set [Status] = 'Verified' where dealid = @DealID and [AuditUserName] = @userName  and IsProjectedPaydown <> 1 and [Status] = 'Exported'
		END
		IF(@verificationFor = 'Projection')
		BEGIN
			UPDATE [IO].[out_FutureFunding] set [Status] = 'Verified' where dealid = @DealID and [AuditUserName] = @userName  and IsProjectedPaydown = 1 and [Status] = 'Exported'
		...
		IF(@verificationFor = 'PIK')
		BEGIN
			UPDATE [IO].[out_PIKPrincipalFunding] set [Status] = 'Verified' where [CRENoteID] = @CRENoteId and  FundingPurpose in ('PIKNC','PIKPP')
		END
		IF(@verificationFor = 'Balloon')
		BEGIN
			UPDATE [IO].[out_PIKPrincipalFunding] set [Status] = 'Verified' where [CRENoteID] = @CRENoteId and  FundingPurpose in ('Balloon')
		END
