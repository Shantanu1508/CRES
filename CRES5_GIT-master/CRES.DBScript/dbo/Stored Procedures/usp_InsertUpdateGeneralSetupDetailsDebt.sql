...
CREATE PROCEDURE [dbo].[usp_InsertUpdateGeneralSetupDetailsDebt] 
(
...
AS
BEGIN
	DECLARE @LookupID int = (Select LookupID from core.lookup where parentid = 3 and [Name]='GeneralSetupDetailsDebt')
	...
	Set @EventID = (Select EventID from [Core].[Event] Where AccountID=@AccountID and EffectiveStartDate = @EffectiveDate and statusID = 1 and EventTypeID = @LookupID )
	...
	ELSE
	BEGIN
		Delete from [Core].[GeneralSetupDetailsDebt] Where EventID = @EventID;
	END

	Insert INTO [Core].[GeneralSetupDetailsDebt] ([EventId],Commitment,InitialMaturityDate,ExitFee,ExtensionFees,[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate])
	VALUES(@EventID,@Commitment,@InitialMaturityDate,@ExitFee,@ExtensionFees,@UserID,getdate(),@UserID,getdate())
