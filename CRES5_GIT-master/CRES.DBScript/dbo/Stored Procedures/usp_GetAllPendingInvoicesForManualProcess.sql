CREATE PROCEDURE [dbo].[usp_GetAllPendingInvoicesForManualProcess] 
(
	@UserID NVARCHAR(256)
)
AS
...
SELECT [InvoiceDetailID]
	  ...
	  and i.InvoiceDetailID not in
	  (select InvoiceDetailID from cre.InvoiceDetailLanding
	  where [Status]='Created' OR [Status]='ReadyToPaid' OR [Status]='Paid')
	  order by d.DealName
END
