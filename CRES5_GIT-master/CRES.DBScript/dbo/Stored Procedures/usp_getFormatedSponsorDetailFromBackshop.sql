CREATE PROCEDURE [dbo].[usp_getFormatedSponsorDetailFromBackshop] 
(
...
BEGIN
	...
	IF EXISTS(Select controlId from @tblResult)
	BEGIN
		Select Distinct d.dealid,t.controlId,t.Sponsor  
	...
	               			From(
								...
								---for prod 			
								 --Select controlId,Sponsor,EmailIDs
								---for QA 			
								 --Select controlId,Sponsor, substring(EmailIDs,1,charindex('@',EmailIDs))+'mailinator.com' as EmailIDs
								 Select controlId,Sponsor, 'rsahu@mailinator.com,shantanu@hvantage.com,skhan@hvantage.com' as EmailIDs
	...
	ELSE
	BEGIN
		Select null as dealid,null as controlId,null as Sponsor,null as EmailIDs  
		From @tblResult
		WHERE 1 = 2
	END
