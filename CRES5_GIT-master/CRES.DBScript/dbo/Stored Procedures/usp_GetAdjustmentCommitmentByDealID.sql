...
--  [dbo].[usp_GetAdjustmentCommitmentByDealID]'b0e6697b-3534-4c09-be0a-04473401ab93','1D3AF6F9-818D-4CB5-BAF5-C41870A802DF'
CREATE PROCEDURE [dbo].[usp_GetAdjustmentCommitmentByDealID]  
(
...
--BEGIN
		...
		---=========================ClosingDate Query==============================------------
		INSERT INTO #TempDataTable(NoteAdjustedCommitmentMasterID,CRENoteID,NoteID,[Type],[Date],TypeText,DealAdjustmentHistory,AdjustedCommitment,TotalCommitment,
		AggregatedCommitment,NoteAdjustedCommitment,NoteTotalCommitment,NoteAggregatedCommitment,Comments,lienposition,[priority],InitialFundingAmount,NoteName,TotalRequiredEquity,TotalAdditionalEquity, Amount,CommitmentType,TotalEquityatClosing)
		...
		SELECT  0,
				...
				(SELECT top 1 ISNULL(TotalEquityatClosing,0) from cre.NoteAdjustedCommitmentMaster where dealid=@DealID and Date = c.ClosingDate and Type = '637')
		...
		TotalCommitment,AggregatedCommitment,NoteAdjustedCommitment,NoteTotalCommitment,NoteAggregatedCommitment,Comments,lienposition,[priority],InitialFundingAmount,NoteName,TotalRequiredEquity,TotalAdditionalEquity,Amount,CommitmentType,TotalEquityatClosing)
		...
		INSERT INTO #TempDataTable(NoteAdjustedCommitmentMasterID,CRENoteID,NoteID,[Type],[Date],TypeText,DealAdjustmentHistory,AdjustedCommitment,TotalCommitment,AggregatedCommitment,NoteAdjustedCommitment,NoteTotalCommitment,NoteAggregatedCommitment,Comments,lienposition,[priority],InitialFundingAmount,NoteName,TotalRequiredEquity,TotalAdditionalEquity,Amount,CommitmentType,TotalEquityatClosing)
		...
		--(CASE WHEN a.Remitdate is not null THEN (-1 * ISNULL(a.BalloonPayment,0)) ELSE ISNULL(-1 * (a.BalloonPayment - ISNULL(BalloonRepayAmount,0)),0) END) as Amount,
		...
		--inner join (
			--Select noteid,(-1 * BalloonPayment) as BalloonPayment,PeriodEndDate from(
		...
		SELECT   nac.NoteAdjustedCommitmentMasterID,
				...
				--ISNULL(nac.ExcludeFromCommitmentCalculation,0),
				0 as TotalEquityatClosing
...
--	END
	...
	----===========PivotingTempTable====================--------------
		...
		SET @query =N'
					SELECT NoteAdjustedCommitmentMasterID,Type,Date,TypeText,DealAdjustmentHistory,AdjustedCommitment,TotalCommitment,AggregatedCommitment,TotalRequiredEquity,TotalAdditionalEquity,Comments,' + @ColPivot1 + ',CommitmentType,TotalEquityatClosing
					FROM (
					SELECT CRENoteID,NoteAdjustedCommitmentMasterID,[Type],[Date],TypeText,DealAdjustmentHistory,AdjustedCommitment,TotalCommitment,AggregatedCommitment,TotalRequiredEquity,TotalAdditionalEquity,Comments,
					ISNULL(Amount,0) as Amount,CommitmentType,TotalEquityatClosing
