--[dbo].[usp_GetWellsViewsAllDataByDealID]   '18-2016'                      
--[dbo].[usp_GetWellsViewsAllDataByDealID]   '18-2260'                 
--[dbo].[usp_GetWellsViewsAllDataByDealID]   '18-1871'               
--[dbo].[usp_GetWellsViewsAllDataByDealID]   '18-0296'               
                          
CREATE Procedure [dbo].[usp_GetWellsViewsAllDataByDealID]                           
...
BEGIN                            
 ...
 SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED                            
  ...
  exec [dbo].[usp_UpdateDealDataForWellsFromBackshopByDealID]  @CREDealID        
...
BEGIN                            
 ...
 (                            
  ...
  --d.Companyname,                              
  REPLACE(n.WF_Companyname,'&','and') as Companyname,          
  n.WF_FirstName as FirstName,          
          
  --d.FirstName,                              
  '' as Lastname,           
  '' as EntityType,         
  'Y'  as IncludeinSearch  ,           
  LEFT(n.WF_StreetName,(PATINDEX('% %',n.WF_StreetName))) StreenNo ,                            
  SUBString(n.WF_StreetName,Charindex(' ',n.WF_StreetName) + 1,LEN(n.WF_StreetName)) StreetName,           
  ...
  n.WF_ZipCodePostal as ZipCodePostal,                              
  REPLACE(n.WF_PayeeName,'&','and') as PayeeName,                              
  n.WF_TelephoneNumber1 as TelephoneNumber1,            
                              
  'N'  as [1098YN]  ,                              
  'Dear Borrower'  as Salutation  ,                 
  'E'  as Language  ,                              
  'N'  as FedIDVerified  ,                              
  n.WF_FederalID1 as FederalID1,                              
  ''  as CIFNumber  ,                              
  'A'  as NegAmTaxBasis  ,                              
  'N'  as IOEWithheld  ,                              
  'Y'  as USCitizen  ,                              
  ...
  (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then TaxEscrowConstant else 0 end) as TaxEscrowConstant,                              
   (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then InsEscrowConstant else 0 end) as InsEscrowConstant,                              
...
''  as InsEscrowBal  ,                              
  ''  as Escrow3Balance  ,                              
  ''  as FHAPMIBalance  ,                             
  n.StubIntOverride as OddDaysIntAmount,                           
                
                  
  (case when n.CurrentBls > 1.0 then 'B' else 'N' end) as OddDaysIntMethod,                       
  --(case when tblNoteFF.IsFF > 0 then 'B' else 'N' end) as OddDaysIntMethod,                         
 ...
 (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then CollectTaxEscrow else 'N' end) as CollectTaxEscrow,                            
(Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then CollectInsEscrow else 'N' end) as CollectInsEscrow,                  
 --(case when tblNoteFF.IsFF > 0 then 'N' else 'Y' end)  as CollectTaxEscrow,                            
  --(case when tblNoteFF.IsFF > 0 then 'N' else 'Y' end)  as CollectInsEscrow,                          
  ...
  Ptype.PropertyCollateralType as  PropertyType,                  
 ...
 n.InitialFundingAmount,                         
  ...
  (                            
   ...
   from [CORE].RateSpreadSchedule rs               INNER JOIN [CORE].[Event] e on e.EventID = rs.EventId                            
  ...
  LEft JOin(                  
  Select dealid, (CASE                  
  WHEN (Select Count(Distinct p.PropertyType) from cre.property p where p.deal_dealid =d.dealid and p.PropertyType is not null and ISNULL(p.IsDeleted,0) <> 1) = 1                  
  THEN (Select top 1 lp.[value] from cre.property p left join core.lookup lp on lp.lookupid = p.PropertyType where p.deal_dealid =d.dealid and p.PropertyType is not null and ISNULL(p.IsDeleted,0) <> 1)                  
  ELSE '7' END                  
  ) as PropertyCollateralType                  
  from cre.deal d                          
   )Ptype on Ptype.dealid = d.dealid                  
                          
  where d.credealid in (@CREDealID)                    
 and ISNULL(n.InitialFundingAmount,0) <> 0    and n.FundedAndOwnedByThirdParty <> 1              
 and acc.statusid = 1                  
                            
 )a                             
order by ISNULL(a.lienposition,99999), a.Priority,a.InitialFundingAmount desc  ,a.VendorLoanNumber                 
...
BEGIN                            
 ...
 (             
  ...
  LoanStatusPriorRep,MethodDorM,Frequency,ARMFormulaCode,CreDealID,dealname,CRENoteID  ,lienposition, Priority,InitialFundingAmount ,statusid                          
  FROM(                            
  ...
  --'' as CurrentPrincipalBalance,                            
  CAst( Cast(tblInterestRate.allincouponrate as decimal(28,5)) as nvarchar(256)) as NetYield,                            
  ...
  'Y' as GroupReport,                            
  'N' as RIMisc1,                       
  CAst( Cast(n.InitialFundingAmount as decimal(28,2)) as nvarchar(256)) as PrincipalBalanceLastrpt,                            
  CAst( Cast(n.InitialFundingAmount as decimal(28,2)) as nvarchar(256)) as PrincipalRepurchase,                          
...
n.InitialFundingAmount,                  
 ...
 where a.credealid in (@CREDealID)                     
  and ISNULL(a.InitialFundingAmount,0) <> 0   and a.FundedAndOwnedByThirdParty <> 1               
 and a.statusid = 1                    
                   
                   
                       
 )b                             
   order by ISNULL(b.lienposition,99999), b.Priority,b.InitialFundingAmount desc,b.VendorLoanNumber                     
...
BEGIN                            
 ...
 (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then CollectTaxEscrow else 'N' end) as CollectTaxEscrow,                             
  ...
  NumberofStories,                              
  (CASE when MeasuredIn = 'sf' then 'S' when MeasuredIn = 'Units' then 'U' else 'M' END) as MeasuredIn,                              
                          
  TotalSquareFeet,                              
  (CASE when TotalSquareFeet is null then TotalRentableSqFt else null END) as TotalRentableSqFt,                              
                            
  p.GroundLease as GroundLease,                            
                            
 --(CASE when MeasuredIn = 'Rooms' then 0 else TotalNumberofUnits END) TotalNumberofUnits,                            
  (CASE when MeasuredIn = 'Units' then TotalNumberofUnits else 0 END) TotalNumberofUnits,                      
                            
  p.NumberOfTenants as NumberOfTenants  ,             
  'G' as OverallCondition,                            
  ...
  '' as ProspectusID ,                            
  '' as PropertyStatus ,                              
  CAst( Cast(p.DealAllocationAmtPCT * 100 as decimal(28,5)) as nvarchar(256))  as Allocation ,                             
                            
  1 as LIENPosition , --p.LIENPosition as LIENPosition ,                               
  '' as AllocofLoanatContra ,                            
  '' as ApprasialValueatContribution ,                            
  '' as ApprasialDateatContribution ,                            
  '' as LastPropContrbutionDT ,                            
  '' as NCFatContribution ,                            
  '' as AllocLoanAmount ,             
                             
   (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then d.CollectInsEscrow else 'N' end)  as CollectINSEscrow ,             
  ...
  '' as CMSAProperyType ,                     
   ...
   n.lienposition as N_lienposition,                  
n.Priority,                  
n.InitialFundingAmount,                    
  ...
  and ISNULL(n.InitialFundingAmount,0) <> 0  and n.FundedAndOwnedByThirdParty <> 1                
 and acc.statusid = 1                  
 and ISNULL(p.PropertyRollUpSW,0) = 0      
 and ISNULL(p.IsDeleted,0) <> 1             
 )a                             
order by ISNULL(a.N_lienposition,99999), a.Priority,a.InitialFundingAmount desc,a.VendorLoanNumber               
...
BEGIN                             
 ...
 (                            
  ...
  'C' as TYPEOFTAX  ,                              
  'CMBS' as TAXAUTHID  ,                            
  (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then TAXBILLSTATUS else 0 end) as TAXBILLSTATUS,                              
  12 as FREQOFDISB  ,                              
  '01012010' as NEXTDISBDATE  ,                              
  '' as TAXAMTDUE  ,                              
  'FL' as LASTBILLTYPEPAID  ,                              
  '' as LASTTAXAMOUNTPAID  ,                              
  (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then d.TaxEscrowConstant else null end) as CURRTAXCONSTANT,                              
  4 as TAXSERVICECODE ,                              
...
BEGIN                            
 ...
 (                            
  ...
  '' as DocNoticeCode  ,                               
   (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then InsuranceBillStatusCode else 0 end)  as BillStatusCode,                            
  Cast(Format( eomonth(ClosingDate,-1) ,'MMddyyyy') as nvarchar(256))  as NextRemitDate,                            
  '' as PremiumFHAAmountDue,                            
                            
   (Case when ROW_NUMBER() OVER (Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber ) = 1 then InsEscrowConstant else null end)  as MonthlyPayment,                            
...
--IF(@ViewName = 'Reserve')                            
BEGIN                            
  Select VendorLoanNumber,RESERVETYPE,ResDescOwnName,ISNULL(MONTHLYPAYMENTAMT,0) as MONTHLYPAYMENTAMT,REMITIOR,IORPLANCODE,IORPRINTCHK,BANKNUMBER,DDANumber,MATURITYDATE,RATE,INVPOOLNUMBER,InvestmentS                 
 from                       
 (                 
  select                          
  n.TaxVendorLoanNumber as VendorLoanNumber,                            
  ...
  Cast(ISNULL(dr.EscrowTypeDesc ,dr.EscrowType) as nvarchar(25)) as ResDescOwnName,                            
                
  (Case when ROW_NUMBER() OVER (PARTITION BY  lResDescOwnName.Value Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber) = 1 then dr.MonthlyBalance else null end)  as MONTHLYPAYMENTAMT,                      
...
CreDealID,                            
  d.dealname,                           
  n.CRENoteID as CRENoteID  ,        
  ROW_NUMBER() OVER (PARTITION BY  lResDescOwnName.Value,dr.MonthlyBalance Order by ISNULL(n.lienposition,99999), n.Priority,n.InitialFundingAmount desc,n.TaxVendorLoanNumber)  as rowno                         
  from cre.note n                            
  inner join Core.Account acc on acc.AccountID=n.Account_AccountID                            
  inner join cre.Deal d on d.DealId=n.DealId                          
  left join [CRE].[DealReserve] dr on dr.loannumber = d.credealid and EscrowType not in ('Taxes','Insurance')                
  left join core.lookup lResDescOwnName on lResDescOwnName.name = dr.EscrowType  and ParentID = 76   --n.ResDescOwnName                
  where d.credealid in (@CREDealID)                   
  and ISNULL(n.InitialFundingAmount,0) <> 0     and n.FundedAndOwnedByThirdParty <> 1           
  and acc.statusid = 1                
 ...
 where a.rowno = 1                         
 order by ISNULL(a.lienposition,99999), a.Priority,a.InitialFundingAmount desc,a.VendorLoanNumber  ,MONTHLYPAYMENTAMT desc            
...
BEGIN                            
 ...
 Cast(Cast((ISNUll(tblnoteRateINdexFloor.PerValue,0) + ISNUll(tblnoteRate.PerValue,0)) * 100 as decimal(28,5)) as nvarchar(256)) as  InterestRateFloor,                                
  Cast(Cast((ISNUll(tblnoteRateINdexCap.PerValue,0) + ISNUll(tblnoteRate.PerValue,0)) * 100 as decimal(28,5)) as nvarchar(256)) as InterestRateCeiling,                          
  ...
  Cast(Cast(IndexRoundingRule as decimal(28,15))/1000000 as decimal(28,5)) as IndexRoundingRule ,                            
  ...
  --2 as Dayslookback  ,                              
  n.Dayslookback as  Dayslookback,                        
  n.Dayslookback as Noticedaysfrreview  ,                             
...
n.InitialFundingAmount,                    
 ...
 LEFT JOIN [CORE].[Lookup] LValueTypeID ON LValueTypeID.LookupID = rs.ValueTypeID                    
  ...
  where a.RNO = 1                            
  ) tblnoteRateINdexFloor on tblnoteRateINdexFloor.NoteId= n.NoteId                          
  ...
  where e.StatusID = (Select LookupID from Core.Lookup where name = 'Active' and parentid = 1)                            
  and LValueTypeID.name in ('Index Cap')                            
 )a                            
  where a.RNO = 1                            
  ) tblnoteRateINdexCap on tblnoteRateINdexCap.NoteId= n.NoteId                         
                            
  left join                             
  (                       Select Distinct [Date],AnalysisID,Value from core.indexes  where IndexType = 245 and AnalysisID  = @AnalysisID                            
