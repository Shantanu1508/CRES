...
Declare @PIKPrincipalPaid int = (Select TransactionTypesID from cre.TransactionTypes where TransactionName = 'PIKPrincipalPaid')  
...
INSERT INTO [#TempPIKINterestPaid] (NoteID,TransactionDate,TransactionTypeId,Amount,DateDue,ServicingAmount,CalculatedAmount,Delta,M61Value,ServicerValue,Ignore,OverrideValue,comments,PostedDate,ServcerMasterID,CreatedBy,CreatedDate,UpdatedBy,UpdatedDate,
TransactionTypeText,TranscationReconciliationID,RemittanceDate,Exception,Adjustment,ActualDelta,OverrideReason,Flag,AmountToBeUpdate,WriteOffAmount,AddlInterest,TotalInterest)  
Select   
 tre.NoteID,   
 tre.TransactionDate,  
 ...
 and TransactionType in ('PIKInterestPaid','ManagementFee','PIKPrincipalPaid','PrepaymentFeeExcludedFromLevelYield','UnusedFeeExcludedFromLevelYield')  
...
from(  
 ...
 where ntd.TransactionTypeText in ('PIKInterestPaid','ManagementFee','PIKPrincipalPaid','PrepaymentFeeExcludedFromLevelYield','UnusedFeeExcludedFromLevelYield')  
)b  
where   
...
INSERT into cre.NoteTransactionDetail(NoteID,TransactionDate,TransactionType,Amount,RelatedtoModeledPMTDate,ServicingAmount,CalculatedAmount,Delta,M61Value,ServicerValue,Ignore,OverrideValue,comments,PostedDate,ServicerMasterID,CreatedBy,CreatedDate,UpdatedBy,UpdatedDate,TransactionTypeText,TranscationReconciliationID,RemittanceDate,Exception,Adjustment,ActualDelta,OverrideReason,WriteOffAmount,AddlInterest,TotalInterest)  
Select NoteID,  
TransactionDate,  
...
and cre.NoteTransactionDetail.TransactionTypeText in ('PIKInterestPaid','ManagementFee','PIKPrincipalPaid','PrepaymentFeeExcludedFromLevelYield','UnusedFeeExcludedFromLevelYield')  
  


Update cre.TranscationReconciliation SET postedDate=getDate() where Transcationid in ( 
	Select tre.Transcationid from cre.TranscationReconciliation tre    
	left join cre.transactiontypes ty on ty.TransactionName = tre.TransactionType  
	Where  tre.deleted=0 and (isnull(tre.M61Value,0) <> 0 or isnull(tre.ServicerValue,0) <> 0  or tre.OverrideValue is not null)  
	and tre.postedDate is null   
	and TransactionType in ('PIKInterestPaid','ManagementFee','PIKPrincipalPaid','PrepaymentFeeExcludedFromLevelYield','UnusedFeeExcludedFromLevelYield')  
	and tre.SplitTransactionid is null
)
