CREATE PROCEDURE [dbo].[usp_CopyFF_FromOtherSource_ForEffectiveDate]
...
Delete from core.FundingSchedule where [EventID] in (Select [EventID] from [Core].[Event] where [AccountID] = @c_accountid and [EventTypeID] = 10 and EffectiveStartDate = @EffectiveDate)

Delete from [Core].[Event] where [AccountID] = @c_accountid and [EventTypeID] = 10 and EffectiveStartDate = @EffectiveDate
...
--FundingSchedule
IF EXISTS (SELECT EventTypeID FROM ##tblEvent WHERE EventTypeID=@FundingSchedule and EffectiveStartDate = @EffectiveDate)
BEGIN
...
[EventId]
	  ...
	  SELECT (SELECT TOP 1
			...
			and se.EffectiveStartDate = @EffectiveDate),
	...
	and e.EffectiveStartDate = @EffectiveDate
