...
CREATE PROCEDURE [dbo].[usp_GetFundingDrawByBusinessdayWF] 
   @NextBDNumber INT
AS
BEGIN
      SET NOCOUNT ON;
...
SET @NextBDDate = (Select Date as enddate from @tblDateRange  where date >= CAST(getdate() as date) order by date OFFSET (@NextBDNumber) ROWS FETCH NEXT 1 ROWS ONLY)
