CREATE PROCEDURE [dbo].[usp_GetAllInvoicesFromInvoiceLanding] 
(
	@UserID NVARCHAR(256)
)
AS
...
SELECT i.[InvoiceDetailID]
	  ...
	  join [CRE].InvoiceDetailLanding il on il.InvoiceDetailID=i.InvoiceDetailID
	  ...
	  and isnull(i.InvoiceNo,'')=''
	  and isnull(il.InvoiceNo,'')<>''
	  and cast([Date] as date)<=cast(getdate() as date) 
	  and  i.DrawFeeStatus = 696 and i.fileName is null
	  and il.Status='Created'
END
