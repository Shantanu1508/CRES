CREATE Procedure [dbo].[usp_GetFeeFunctionsConfigLiability]
@UserID NVarchar(255)
AS
...
BEGIN
	...
	IsUsedInFeeSchedule = Case when exists(select 1 from [CRE].[FeeSchedulesConfigLiability] where FeeFunctionID=FF.FunctionNameID) then 1 else 0 end	
