...
BEGIN
	...
	--DECLARE @tLookupRateSpreadSchedule as table
	--(
	--ValueTypeID int,
	--[Name] nvarchar(100)
	--)
	--insert into @tLookupRateSpreadSchedule
	--values
	--(150,'rate'),
	--(151,'Spread'),
	--(154,'index_floor'),
	--(157,'amort_spread'),
	--(596,'reference_rate')
	--get all deal notes with effective date
	--insert into @tNoteEffectiveDates
	--select distinct CRENoteID,n.NoteID,tbl.effective_dates from cre.note n join [CORE].[Account] ac 
	--on n.Account_AccountID=ac.AccountID 
	--join
	--(
	--Select 
	--	EffectiveStartDate as effective_dates,NoteID
	--	from [CORE].[Event] eve
	...
	--	INNER JOIN [CORE].[Account] acc ON acc.AccountID = n.Account_AccountID
	--	inner join cre.deal d on d.DealID=n.DealID
	--	left join core.lookup lEventType on lEventType.lookupid = eve.EventTypeID	
	--	where  acc.IsDeleted=0
	--	and isnull(eve.StatusID,1) = 1
	--	and lEventType.name in (
	--	'DefaultSchedule',
	--	'FeeCouponSchedule',
...
from
(
	select distinct(n.CRENoteID) as CRENoteID,@CREDealID as CREDealID
	from [CORE].[Event] eve
	INNER JOIN [CRE].[Note] n ON n.Account_AccountID = eve.AccountID
	INNER JOIN [CORE].[Account] acc ON acc.AccountID = n.Account_AccountID
	inner join cre.deal d on d.DealID=n.DealID
