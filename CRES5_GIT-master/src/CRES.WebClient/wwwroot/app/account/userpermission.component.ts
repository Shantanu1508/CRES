...
import { User, AppConfig } from '../core/domain/user';
...
import { SchedulerConfig } from '../core/domain/SchedulerConfig';
import { SchedulerParam } from '../core/domain/SchedulerParam';
...
export class userpermission {
    _chkShowAllActiveInactiveUsers = false;
    public rolelist: any;
    public RoleID: any;
            ...
            if (NewItemsAdded.length > 0 || NewItemsEdited.length > 0) {
                ...
                for (var i = 0; i < NewItemsAdded.length; i++) {
                    ...
                    if (!(Number(NewItemsAdded[i].RoleName.toString()).toString() == "NaN" || Number(NewItemsAdded[i].RoleName) == 0)) {
                        ...
                        var filteredarray = this.rolelist.filter(x => x.RoleID == NewItemsAdded[i].RoleName)
                        if (filteredarray.length != 0) {
                            NewItemsAdded[i].RoleID = NewItemsAdded[i].RoleName;
                        }
                        else {
                            var filteredarrayNew = this.rolelist.filter(x => x.RoleName == NewItemsAdded[i].RoleName)
                    ...
                    if (!(Number(NewItemsAdded[i].Status).toString() == "NaN" || Number(NewItemsAdded[i].Status) == 0)) {
                        ...
                        var filteredarray = this.lstStatus.filter(x => x.LookupID == NewItemsAdded[i].Status)
                        if (filteredarray.length != 0) {
                            NewItemsAdded[i].StatusID = Number(NewItemsAdded[i].Status);
                        }
                        else {
                            var filteredarrayNew = this.lstStatus.filter(x => x.Name == NewItemsAdded[i].Status)
                            if (filteredarrayNew.length != 0) {
                    ...
                    if (!(NewItemsAdded[i].TimeZone == null || NewItemsAdded[i].TimeZone == undefined || NewItemsAdded[i].TimeZone == "")) {
                        var filteredarray = this.lsttimezone.filter(x => x.TimeZoneID == NewItemsAdded[i].TimeZone)
                        if (filteredarray.length > 0) {
                ...
                for (var i = 0; i < NewItemsEdited.length; i++) {
                    ...
                    if (!(Number(NewItemsEdited[i].RoleName.toString()).toString() == "NaN" || Number(NewItemsEdited[i].RoleName) == 0)) {
                        ...
                        var filteredarray = this.rolelist.filter(x => x.RoleID == NewItemsEdited[i].RoleName)
                        if (filteredarray.length != 0) {
                    ...
                    if (!(Number(NewItemsEdited[i].Status.toString()).toString() == "NaN" || Number(NewItemsEdited[i].Status) == 0)) {
                        ...
                        var filteredarray = this.lstStatus.filter(x => x.LookupID == NewItemsEdited[i].Status)
                        if (filteredarray.length != 0) {
                            NewItemsEdited[i].StatusID = Number(NewItemsEdited[i].Status);
                        }
                    }
                    ...
                    if (!(NewItemsEdited[i].TimeZone == null || NewItemsEdited[i].TimeZone == undefined || NewItemsEdited[i].TimeZone == "")) {
                        var filteredarray = this.lsttimezone.filter(x => x.TimeZoneID == NewItemsEdited[i].TimeZone)
                        if (filteredarray.length > 0) {
                ...
                this.membershipService.AddUpdateUser(this.UsersToUpdate).subscribe(res => {
                    if (res.Succeeded) {
                        ...
                        this._ShowSuccessmessage = 'User Add/Update failed.';
                        this._ShowSuccessmessagediv = true;
                        setTimeout(function () {
                            this._ShowSuccessmessagediv = false;
                        }.bind(this), 2000);
    ...
    private _buildDataMap(items, key, value): wjcGrid.DataMap {
        var map = [];
        if (items) {
    ...
    CreateRole(): void {
        if (this.role.NewRoleName.toString() == "") {
            ...
            var rolename = this.rolelist.filter(x => x.RoleName.toLowerCase() == this.role.NewRoleName.toLowerCase())
    ...
    GetAllowBasicLogin() {
        this.permissionService.GetAllAppSetting().subscribe(res => {
            if (res.Succeeded) {
                ...
                if (data.find(x => x.Key == "AllowBasicLogin").Value == "0") {
                    this._isChecked = false;
                }
                else {
                    this._isChecked = true;
                }
                if (data.find(x => x.Key == "AllowDebugInCalc").Value == "0") {
                    this._isCalcChecked = false;
                }
                else {
                    this._isCalcChecked = true;
                }
                if (data.find(x => x.Key == "AllowCalcBooster").Value == "0") {
                    this._iscalcboosterChecked = false;
                }
                else {
                    this._iscalcboosterChecked = true;
                }
                if (data.find(x => x.Key == "AllowFFSaveJsonIntoBlob").Value == "0") {
                ...
                if (data.find(x => x.Key == "AllowBackshopFF").Value == "0") {
    ...
    UpdateAllowBasicLogin(valuechecked, Key) {
        this._isCalcListFetching = true;
    ...
    CheckDuplicateEmailLogin(lstData, rwNum): boolean {
        try {
            var i;
    ...
    showForceLogoutDialog(resetRowIndex) {
        this._forceLogout = {
        ...
        let modalForceLogout = document.getElementById('myModalForceLogout');
    ...
    onChangeWFApprover(e, userdetail) {
            ...
            for (var k = l + 1; k < this.lstApproverUser.length; k++) {
                if (this.lstApproverUser[k].ModuleName != "" || this.lstApproverUser[k].ModuleName != null || this.lstApproverUser[k].ModuleName != undefined) {
                    ...
                    if (!(Number(this.lstApproverUser[k].ModuleName).toString() == "NaN" || Number(this.lstApproverUser[k].ModuleName) == 0)) {
                        approvertype = this.lstapprover.find(x => x.ModuleId == Number(this.lstApproverUser[k].ModuleName)).ModuleName
    ...
    SendWFApproverEmail(userdetails) {
        var envname: any;
    ...
    DeleteWFApprover(userdeletedetails) {
        this._isCalcListFetching = true;
        this._deletewfapprover = {
