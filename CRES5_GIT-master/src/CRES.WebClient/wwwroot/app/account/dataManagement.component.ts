import { Component, Inject, OnInit, ViewChild, AfterViewInit, Input, Output, EventEmitter } from "@angular/core";
...
import { ruletype } from "../core/domain/ruleType";


import { dealService } from '../core/services/dealservice';
import { UtilityService } from '../core/services/utilityService';
import { NoteService } from '../core/services/NoteService';
import { JsonTemplate, Note } from "../core/domain/note";
...
import { Servicer, TemplateName } from "../core/domain/Note";

import { changepowerbipasswrd } from "../core/domain/changepassword";
import { scenarioService } from "../core/services/scenarioService";
import { WjNavModule, WjTreeView } from 'wijmo/wijmo.angular2.nav';
import { WjInputModule, WjMenu } from 'wijmo/wijmo.angular2.input';
...
export class DataManagement {
    ...
    public lstdeal: any;
    private _changepowerbipasswrd: changepowerbipasswrd;
    private _note: Note;
    ...
    private _isShowNoteGrid: boolean = false;
    private _isShowDealGrid: boolean = false;
    ...
    public repaymentdropdate: number;
    public servicermasterid: number;
    public servicer: Servicer;
    public _jsontemplate: JsonTemplate;
    public _templatename: TemplateName;
    public newjsontemplatename: any = '';
    public JsonTemplateValue: any = '';
    public JsonTemplateName: any = [];
    public lstTemplatename: any = [];
    public changedCalendarId: any;
    public changedjsontemplateValue: any;
    ...
    private _isShowaddnewvalidation: boolean = false;
    ...
    constructor(public notesvc: NoteService, public utilityService: UtilityService, public notificationService: NotificationService, public dealSrv: dealService, public membershipService: MembershipService,
        ...
        ruletype
        this._note = new Note('');
        this._changepowerbipasswrd = new changepowerbipasswrd();
        this._templatename = new TemplateName();
    ...
    ServicerNameChange(newvalue, servicerboxname): void {
    ...
    GetAllLookups(): void {
        this.dealSrv.getAllLookup().subscribe(res => {
            ...
            this.lstCalculated = data.filter(x => x.ParentID == "2");
            this.lstCashflowdownlaod = data.filter(x => x.ParentID == "2");
            this.lstAllowcalculationOverride = data.filter(x => x.ParentID == "2");
            this.lstGAAPCalculations = data.filter(x => x.ParentID == "2");
            this.lstServicingRecocilliation = data.filter(x => x.ParentID == "2");
    ...
    AssignvaluestoTransactiontypeDropdown(data, e) {
        var datatext;
        if (data == 4 || data == "4") {
            datatext = "N";
        }
    ...
    CalendarNameChange(newvalue) {
        this.changedCalendarId = newvalue;
        var calendarname = this.lstCalendarName.find(x => x.HolidayMasterID.toString() == newvalue).CalendarName;
    ...
    checkDuplicateHolidayDates(data, mode) {
        var founddates = '';
    ...
    saveHolidayDate(dtholidaydates) {
    ...
    getPowerBIPassword() {
        ...
        this.permissionService.getpowerBIpassword().subscribe(res => {
            if (res.Succeeded == true) {
                this._isShowLoader = false;
                this._changepowerbipasswrd.Value = res.appconfigpowerBI;
            }
        });
    }
    ...
    updatepowerbipassword() {
        ...
        this.permissionService.UpdatePowerBIPassword(this._changepowerbipasswrd).subscribe(res => {
            if (res.Succeeded) {
                this._isShowLoader = false;
                this._ShowSuccessmessage = " Password Saved successfully.";
                this._ShowSuccessmessagediv = true;
                this._changepowerbipasswrd.Value = "";
...
@NgModule({
    ...
    imports: [FormsModule, CommonModule, RouterModule.forChild(routes), WjGridModule, WjInputModule, WjGridFilterModule, Ng2FileInputModule.forRoot(), WjNavModule],
