...
exports.UtilsFunctions = void 0;
var UtilsFunctions = /** @class */ (function () {
    function UtilsFunctions() {
    }
    UtilsFunctions.prototype.convertDateToBindable = function (date) {
        var dateObj = null;
        if (date) {
    ...
    UtilsFunctions.prototype.convertDateToBindableWithformatchar = function (date, formatchar) {
        var dateObj = null;
        if (date) {
            if (typeof (date) == "string") {
                date = date.replace("Z", "");
    ...
    UtilsFunctions.prototype.convertDateWithoutSlash = function (date) {
        var dateObj = null;
        if (date) {
            if (typeof (date) == "string") {
                date = date.replace("Z", "");
    ...
    UtilsFunctions.prototype.convertDateToUTC = function (date) {
        var dateObj = null;
        if (date) {
            if (typeof (date) == "string") {
                date = date.replace("Z", "");
    ...
    UtilsFunctions.prototype.getTwoDigitString = function (number) {
        if (number.toString().length === 1)
            return "0" + number;
        return number;
    };
    UtilsFunctions.prototype.GetDefaultValueNumber = function (val) {
        if (isNaN(val) || val == null) {
            return 0;
        }
        return val;
    };
    UtilsFunctions.prototype.formatNumberforTwoDecimalplaces = function (data, currencytext) {
    ...
    UtilsFunctions.prototype.convertToTwoDecimalPlaces = function (data) {
        if (data == null || data === undefined) {
            data = 0;
        }
        var num = parseFloat(data).toFixed(2);
    ...
    UtilsFunctions.prototype.GetCurrencySymbol = function (CurrencyText) {
    ...
    UtilsFunctions.prototype.formatNumberTopercent = function (num) {
        var s;
    ...
    UtilsFunctions.prototype.convertDatetoGMT = function (date) {
        var _dtUTCHours;
        var _userOffset;
        var _centralOffset;
        var _date = new Date();
    ...
    UtilsFunctions.prototype.convertDateToBindableWithTime = function (date) {
    ...
    UtilsFunctions.prototype.getnextbusinessDate = function (sDate, noofDays, addorsub, ListHoliday) {
        var _this = this;
        if (sDate) {
            var i = 0;
            while (i < Math.abs(noofDays)) {
                ...
                if (addorsub == true) {
                    if (daycnt == 6 || daycnt == 0
                        || ListHoliday.filter(function (x) { return _this.convertDateToBindable(x.HolidayDate) == formateddate && x.HolidayTypeID == 411; }).length > 0) {
    ...
    ///string utils
    UtilsFunctions.prototype.SubstringFromLast = function (str) {
        if (str != "") {
    ...
    //number utils
    UtilsFunctions.prototype.ConverttoFloat = function (d) {
        var number = 0;
    ...
    UtilsFunctions.prototype.DivideIfNotZero = function (numerator, denominator) {
        try {
