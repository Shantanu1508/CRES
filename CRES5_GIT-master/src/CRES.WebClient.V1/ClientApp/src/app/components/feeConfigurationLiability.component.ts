...
declare var $: any;
@Component({
  templateUrl: "./feeConfigurationLiability.html",
  providers: [NoteService, dealService, feeconfigurationService, PermissionService]
})


export class FeeConfigurationComponentLiability {
  ...
  GetAllFeeFunction(): void {
    if (this.callFeeAmount) {
      ...
      try {
        ...
        this.feeconfigurationSrv.GetAllFeeFunctionLiability().subscribe(res => {
          if (res.Succeeded) {
  ...
  GetAllFeeAmount(): void {
    ...
    if (this.callBaseAmount) {
      ...
      try {
        this.feeconfigurationSrv.GetAllFeeAmountLiability().subscribe(res => {
          if (res.Succeeded) {
            this.listfeeamount = []
  ...
  SaveFeeConfig(): void {
    ...
    if (isduplicatefunctionName == true || isduplicatefeetypeName == true) {
      ...
      try {
        this.feeconfigurationSrv.SaveFeeConfigLiability(this._feeConfig).subscribe(res => {
          if (res.Succeeded) {
            ...
            if (window.location.href.indexOf("feeconfigurationliability/a") > -1) {
              returnUrl = returnUrl.toString().replace('feeconfigurationliability/a', 'feeconfigurationliability');

            }
            else if (returnUrl.indexOf("feeconfigurationliability") > -1) {
              returnUrl = returnUrl.toString().replace('feeconfigurationliability', 'feeconfigurationliability/a');
            }
  ...
  DeleteFeeScheduleConfig() {
    try {
      this._isFeeFunctionListFetching = true;
      this.feeconfigurationSrv.DeleteScheduleConfigLiability(this.FeeTypeGuID).subscribe(res => {
        if (res.Succeeded) {
  ...
  DeleteFeeFunctionConfig() {
    try {
      this._isFeeFunctionListFetching = true;
      this.feeconfigurationSrv.DeleteFunctionConfigLiability(this.FeeTypeGuID).subscribe(res => {
        if (res.Succeeded) {
...
const routes: Routes = [
  { path: '', component: FeeConfigurationComponentLiability }]

@NgModule({
  imports: [FormsModule, CommonModule, RouterModule.forChild(routes), WjGridModule, WjGridFilterModule],
  declarations: [FeeConfigurationComponentLiability]
})

export class feeConfigurationLiabilityModule { }
