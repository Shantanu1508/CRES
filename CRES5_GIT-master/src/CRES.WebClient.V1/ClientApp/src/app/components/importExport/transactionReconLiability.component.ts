...
@Component({
  templateUrl: "./transactionReconLiability.html",
  providers: [TranscationreconciliationService, FileUploadService, PermissionService, dealService]
})

export class TranscationreconLiabilityComponent extends Paginated {
  lstServicer: any;
  ...
  GetAllServicer(): void {
    this.transserv.getAllServicerLiability().subscribe(res => {
      if (res.Succeeded) {
  ...
  GetAllTranscation(type: any = ""): void {
    ...
    if (!this._isFilterapply) {
      ...
      this.transserv.getAllTranscationLiability(type, this._pageIndex, this._pageSize).subscribe(res => {
        if (res.Succeeded) {
  ...
  RefreshM61Amount() {
    ...
    this.GetAllTranscation(showallRefreshAmount);
  }
...
const routes: Routes = [
  ...
  { path: '', component: TranscationreconLiabilityComponent }]

@NgModule({
  imports: [FormsModule, CommonModule, RouterModule.forChild(routes), WjCoreModule, WjGridModule, WjInputModule, WjGridFilterModule],
  declarations: [TranscationreconLiabilityComponent]
})

export class TranscationreconLiabilityModule {
