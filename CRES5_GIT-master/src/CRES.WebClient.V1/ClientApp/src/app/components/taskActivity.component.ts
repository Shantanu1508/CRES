...
import { TaskManagement, TaskComment } from "../core/domain/taskManagement.model";
...
export class TaskActivityComponent {
  ...
  public _taskComment: TaskComment;
  public _Showmessagediv: boolean = false;

  public _TaskFetching: boolean = false;
    ...
    try {
      ...
      this.taskManagerService.InsertUpdateTask(this._taskManagement).subscribe(res => {
        if (res.Succeeded) {
          ...
          Notificationmessage = _module + '|*|' + Notificationmessage + '|*|' + appsettings.Notificationsettings._notificationenvironment + '|*|' + _lststrSubscibeduser;
