...
namespace CRES.TestAutoMation.TestCases
{
    ...
    public class Parallel_Notes_Calculation : BaseClass
    {
        ...
        public Deal deal;
        readonly string browser = Browser();
        readonly string headless = HeadlessDriver();
        readonly string env = GetEnvironment();
...
public voud MailSend()
{
        ...
        public void noteCalculation(List<DealDataContract> _lstDeal, string NoteID, string Noteid_Href, IWebDriver driver)
        {
            ...
            string subLoginUrl = GetLoginUrlNew();
            string LoginUrl = BaseUrl + subLoginUrl;
            string dealfunding = BaseUrl + DealFunding();
            string dashboard = BaseUrl + GetDashboardUrl();
                    ...
                    try
                    {
                        ...
                        if (CalculationManagerTitle == "Calculation Manager")
                        {
                            ...
                            try
                            {
                                ...
                                Thread.Sleep(3000);
                                //string times = NotesCalculation.Timestamp();
                                CreateExcelDataTableNewForInput((DataTable)JsonConvert.DeserializeObject(JsonConvert.SerializeObject(ListNoteCalculation), typeof(DataTable)), "InputNoteIds");
                                Thread.Sleep(7000);
                        ...
                        string runForAllDeal = TestAllDealForGenerateFunding();
                        int browserCount = BrowserCount();
